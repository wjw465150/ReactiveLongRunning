...
public class IdeLauncher extends Launcher {
   ...
   * started.
   *
   * @param options the configured Vert.x options. Modify them to customize the
   *          Vert.x instance.
   */
  @Override
  public void beforeStartingVertx(VertxOptions options) {
    ...
    if (System.getProperties().getProperty("vertx.options.blockedThreadCheckInterval") != null) {
      blockedThreadCheckInterval = Long
          .valueOf(System.getProperties().getProperty("vertx.options.blockedThreadCheckInterval"));
    }
    options.setBlockedThreadCheckInterval(blockedThreadCheckInterval);
  }

  /**
   * Hook for sub-classes of {@link Launcher} after the vertx instance is
   * started.
   *
  ...
  public void afterStartingVertx(Vertx vertx) {
    if (System.getProperties().getProperty("profile") == null || System.getProperties()
        .getProperty("profile")
        .equalsIgnoreCase("dev")) { //说明在IDE里运行的,这时候等待命令行输入quit来优雅的退出!
